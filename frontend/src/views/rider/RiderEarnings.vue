...
import { getDeliveryList, getEarningsStats } from '@/api/rider'
...
const loadEarningsRecords = async () => {
  loading.value = true
  try {
    const res = await getDeliveryList('3')
    const allRecords = (res || []).map(order => ({
      orderId: order.orderId,
      orderNo: order.orderNo || order.orderId,
      amount: order.fee || 0,
      type: 1,
      createTime: order.deliveryTime || order.createTime,
      remark: '配送订单'
    }))
